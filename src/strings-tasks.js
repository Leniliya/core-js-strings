...
function isPalindrome(str) {
  ...
  const filteredStr = lowStrArr.join('');
  const reversedStr = lowStrArr.reverse().join('');
...
function findLongestWord(sentence) {
  ...
  const sorted = words.sort((curr, next) => {
...
function invertCase(str) {
  const isLowCase = (char) => char === char.toLowerCase();
  const arr = str.split('');
  return arr
    .map((item) => {
      if (isLowCase(item)) {
